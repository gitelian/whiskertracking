function [headerInfo, imgOut] = Norpix2MATLABopenSingleFrame(fileName,frame2show)
...
if nargout > 1
    ...
    imgOut = zeros(imSize(2),imSize(1),'single'); % deleted 3rd argument: headerInfo.AllocatedFrames
    ...
    if frame2show == -1
        nread = headerInfo.AllocatedFrames-1; % frames are zero indexed
    else
        nread = frame2show-1;
    end
    
    % deleted: while loop that iterates through all movie frames
    ...
    % deleted part that iterates through imgOut when imgOut has more than 1
    % frame: imgOut(:,:,nread+1) = ...;
    ...
    headerInfo.timestamp{1} = [datestr(tmp) ':' num2str(tmp2(1)),num2str(tmp2(2))];
    %headerInfo.timestamp{nread + 1}

    % deleted counter for while loop: nread = nread + 1;
