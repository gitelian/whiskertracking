...
%         pause(0.025)
    ...
    text(v.Width*0.75, v.Height*0.90,['Time (s): ' num2str(k/500, '%.3f')],'Color','white','FontSize',14)
    
    %% if frame has whisker traces in the object boundary add a red dot in
    % the TOP RIGHT
    if exist('frm', 'var') & find(frm{mov} == k)
        plot(v.Width*0.90, v.Height*0.10, 'r*', 'markers', 12);
    end
