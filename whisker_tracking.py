...
##### Make set-point vs runspeed plots #####
    ...
    camTime, set_point_list, amp_list, vel_list = get_wt_run_vals(wtmat, trials_ran_dict,
...
#################### run speed difference tuning curve   ######################
    ...
    for i in range(9):
        ...
        sp_error_diff.append(np.sqrt(var1/n1 + var2/n2)*sp.stats.t.ppf((1+0.95)/2.0, n1+n2-2))
    x = np.arange(1, 10)
    # plot tuning curve
    ...
    for i in range(9):
        ...
        var1 = np.nanvar(vl_samples_list[i])
        var2 = np.nanvar(vl_samples_list[i+9])
        vl_error_diff.append(np.sqrt(var1/n1 + var2/n2)*sp.stats.t.ppf((1+0.95)/2.0, n1+n2-2))
    x = np.arange(1, 10)
    # plot tuning curve
