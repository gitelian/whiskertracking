...
figure;
barweb(reshape(mean_setpoint, 7, 2), reshape(sem_setpoint, 7, 2));
ylim([90 160])
xlabel('Positions')
ylabel('Degrees')
...
figure;
barweb(reshape(mean_amp, 7, 2), reshape(sem_amp, 7, 2));
xlabel('Positions')
ylabel('Degrees')
...
subplot(2,1,2)
scatter(speed_and_whisks{pos+7}(:,1), speed_and_whisks{pos+7}(:,2), 'ko')
xlim([0 100])
ylim([90 180])
xlabel('Speed cm/sec')
ylabel('Degrees')
